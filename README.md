...
# TP : Asservissement de vitesse d’une machine à courant continu par PID numérique (STM32)


# 1. Objectifs pédagogiques

Ce TP a pour objectif la mise en œuvre d’un asservissement de vitesse d’une machine à courant continu (MCC) à l’aide d’un microcontrôleur STM32.
...
2. le calcul du correcteur PID
3. la génération du signal PWM

Schéma fonctionnel :

Consigne vitesse → Correcteur PID → PWM → MCC → Capteur vitesse → STM32
...
On note Ts la période d’échantillonnage.
...
Le programme principal est fourni. Les étudiants doivent compléter le calcul du PID.
...
Augmenter progressivement Kp jusqu’à obtenir des oscillations permanentes.
